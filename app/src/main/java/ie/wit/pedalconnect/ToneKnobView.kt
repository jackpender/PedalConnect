...
import kotlinx.android.synthetic.main.tone_knob_view.view.*

class ToneKnobView @JvmOverloads constructor(
    ...
    var toneListener: ToneKnobListener? = null
    var value = 50
    private var toneKnobDrawable: Drawable? = null
...
//    var knobImageView = findViewById<ImageView>(R.id.knobImageView) as ImageView
    ...
    init {
        ...
        LayoutInflater.from(context)
            .inflate(R.layout.tone_knob_view, this, true)

        context.theme.obtainStyledAttributes(
            attrs,
            R.styleable.ToneKnobView,
            0,
            0
        ).apply {
            try {
                minValue = getInt(R.styleable.ToneKnobView_toneMinValue, 0)
                maxValue = getInt(R.styleable.ToneKnobView_toneMaxValue, 100) + 1
                divider = 300f / (maxValue - minValue)
                value = getInt(R.styleable.ToneKnobView_toneInitialValue, 50)
                toneKnobDrawable = getDrawable(R.styleable.ToneKnobView_toneKnobDrawable)
//                knobImageView.setImageDrawable(knobDrawable)
    ...
    override fun onScroll(e1: MotionEvent, e2: MotionEvent, distanceX: Float, distanceY: Float)
        ...
        if (rotationDegrees >= -150 && rotationDegrees <= 150) {
            ...
            if (toneListener != null) toneListener!!.onToneRotate(value)
        }
        return true
    }
    ...
    private fun setKnobPosition(angle: Float) {
        val matrix = Matrix()
        toneKnobImageView.scaleType = ImageView.ScaleType.MATRIX
        matrix.postRotate(angle, width.toFloat() / 2, height.toFloat() / 2)
        toneKnobImageView.imageMatrix = matrix
    }

    interface ToneKnobListener {
        fun onToneRotate(value: Int)
    }
}